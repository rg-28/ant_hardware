...
float linear_x_pid(float twist_linear_x)
{
  float Kp=10,Ki=0,Kd=0;
  float Iterm=0,error=0,p_error=0;
  p_error = error;
  error = twist_linear_x - sensor_vel.linear.x;
  Iterm = (Iterm + error)*Ki;
  float linear_x_out = (Kp*error) + Iterm + (Kd*(error - p_error));
  
  return linear_x_out;
}

float angular_z_pid(float twist_angular_z)
{
  float Kp=0,Ki=0,Kd=0;
  float Iterm=0,error=0,p_error=0;
  p_error = error;
  error = twist_angular_z - sensor_vel.angular.z;
  Iterm = (Iterm + error)*Ki;
  float angular_z_out = (Kp*error) + Iterm + (Kd*(error - p_error));
  
  return angular_z_out;
}
...
void cmdvelCB(const geometry_msgs::Twist& twist)
{
  ...
  float linear_x_gain=linear_x_pid(twist.linear.x);
  float angular_z_gain=angular_z_pid(twist.angular.z);
  left_wheel_data=gain*((twist.linear.x + linear_x_gain) - (twist.angular.z + angular_z_gain)*L);
  right_wheel_data=gain*((twist.linear.x + linear_x_gain) + (twist.angular.z + angular_z_gain)*L);
